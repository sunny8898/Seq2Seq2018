...
class Seq2SeqModel(object):
    def __init__(self,
        ...
        with tf.variable_scope("training", reuse = tf.AUTO_REUSE):
            ...
            tmp = tf.sigmoid(tf.reduce_sum(self.decoder_logits, 0)) # [T, B, V] -> [B, V]
            ...
            self.print_ops.append(tf.Print(tmp, [tf.shape(tmp), tmp], 
                                           "after multi with one hot:", summarize = 10000)) 

            self.bow_loss = tf.reduce_sum(tmp * self.target_weights) / tf.reduce_sum(self.target_weights)
